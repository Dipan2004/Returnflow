...
def test_flywheel_returns_empty_metrics(client: TestClient) -> None:
    resp = client.get("/dashboard/flywheel")
...
def test_metrics_with_custom_period(client: TestClient) -> None:
    resp = client.get("/dashboard/metrics?period=last_7_days")
    assert resp.status_code == 200
