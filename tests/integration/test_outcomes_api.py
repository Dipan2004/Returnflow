...
def test_get_outcome_not_found(client: TestClient) -> None:
    resp = client.get("/outcomes/NOTEXIST")
    assert resp.status_code == 404


def test_accept_not_found(client: TestClient) -> None:
    resp = client.post("/buyers/buyer_1/accept/NOTEXIST")
    assert resp.status_code == 404


def test_reject_not_found(client: TestClient) -> None:
    resp = client.post(
        "/buyers/buyer_1/reject/NOTEXIST",
        json={"reason": "bad item"},
    )
    assert resp.status_code == 404
